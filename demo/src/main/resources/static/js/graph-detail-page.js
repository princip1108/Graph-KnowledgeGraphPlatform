(function () {
    ...
    async function loadGraphData(graphId) {
        ...
        try {
            ...
            // Load nodes
            const nodesResponse = await fetch(`/api/graph/${graphId}/visualization`);
            const vizData = nodesResponse.ok ? await nodesResponse.json() : { nodes: [], links: [] };
            ...
            return {
                ...
                categoryName: vizData.categoryName || graphInfo.categoryName,
                domain: vizData.domain || graphInfo.domain
            };
        } catch (error) {
    ...
    function updatePageWithGraphData(data) {
        ...
        // Update domain category display
        updateGraphDomain(data);
    ...
    async function checkLogin() {
        try {
            ...
            if (data.authenticated && data.user) {
                ...
                // Re-update uploader info to check visibility
                if (window.currentGraphData.uploaderId) {
                    updateUploaderInfo(window.currentGraphData);
                }
            }
        } catch (e) { console.error('Auth check failed', e); }
    }

    window.sendGraphPrivateMessage = function () {
        if (!requireLogin('发送私信')) return;
        const uploaderId = window.currentGraphData.uploaderId;
        const uploaderName = window.currentGraphData.uploaderName;

        if (!uploaderId) return;

        // Open Modal
        document.getElementById('msgRecipientId').value = uploaderId;
        document.getElementById('msgRecipientName').value = uploaderName;
        document.getElementById('sendMessageModal').showModal();
    };

    window.executeSendMessage = function () {
        const receiverId = document.getElementById('msgRecipientId').value;
        const content = document.getElementById('msgContent').value;

        if (!content.trim()) { window.showNotification('请输入消息内容', 'warning'); return; }
        ...
        fetch('/api/messages', {
            ...
            .then(data => {
                if (data.success) {
                    window.showNotification('发送成功', 'success');
                    document.getElementById('msgContent').value = '';
                    document.getElementById('sendMessageModal').close();
                } else {
                    window.showNotification(data.error || '发送失败', 'error');
                }
            })
            .catch(() => window.showNotification('发送失败', 'error'));
    };

    // Update uploader info (Modified to show message button)
    function updateUploaderInfo(data) {
        ...
        const messageBtn = document.getElementById('graphMessageBtn');
        ...
        // FIX: Ensure ID is correct and URL is profile.html?id=...
        // Also check if profile.js handles this param (assumed yes based on context, if not user will report)
        if (data.uploaderId && uploaderLink) {
            uploaderLink.href = '/user/profile.html?id=' + data.uploaderId;
        }

        // Show/Hide Message Button
        // Logic: Show if (not logged in) OR (logged in AND not self)
        if (messageBtn && data.uploaderId) {
            const isSelf = currentUser && currentUser.userId === data.uploaderId;
            if (!isSelf) {
    ...
    // Update graph domain category display
    function updateGraphDomain(data) {
        const sectionEl = document.getElementById('graphDomainSection');
        const domainEl = document.getElementById('graphDomain');
        if (!sectionEl || !domainEl) return;

        // Check if categoryName exists in data
        if (data.categoryName) {
            domainEl.textContent = data.categoryName;
            sectionEl.classList.remove('hidden');
        } else if (data.domain) {
            domainEl.textContent = data.domain;
            sectionEl.classList.remove('hidden');
        } else if (data.category) {
            // Fallback to category code if name not available
            const categoryMap = {
                'ai': '人工智能',
                'medical': '医疗健康',
                'finance': '金融经济',
            ...
            domainEl.textContent = categoryMap[data.category] || data.category;
            sectionEl.classList.remove('hidden');
        } else {
    ...
    // Load related posts for this graph
    async function loadRelatedPosts(graphId) {
        const container = document.getElementById('relatedPostsContainer');
        if (!container || !graphId) return;

        try {
            const response = await fetch(`/api/posts/related?graphId=${graphId}&size=8`);
            ...
            if (!data.success || !data.posts || data.posts.length === 0) {
                container.innerHTML = `
                    <div class="col-span-full flex flex-col items-center justify-center py-8">
                        <span class="iconify text-base-content/30" data-icon="heroicons:chat-bubble-left-right" data-width="48"></span>
                        <p class="mt-3 text-sm text-base-content/50">暂无相关帖子</p>
                        <a href="#" class="btn btn-primary btn-sm mt-3" onclick="createPostForGraph()">发布第一篇帖子</a>
                    </div>`;
                return;
            }

            container.innerHTML = data.posts.map(function(post) {
                var timeStr = post.uploadTime ? new Date(post.uploadTime).toLocaleDateString('zh-CN') : '';
                var badge = post.isDirectlyRelated ? '<span class="badge badge-primary badge-sm">关联</span>' : '';
                return '<div class="card bg-base-100 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onclick="window.location.href=\'/community/post_detail.html?id=' + post.postId + '\'">' +
                    '<div class="card-body p-4">' +
                        '<div class="flex items-start justify-between gap-2">' +
                            '<h3 class="font-semibold text-sm line-clamp-2 flex-1">' + (post.postTitle || '无标题') + '</h3>' +
                            badge +
                        '</div>' +
                        '<p class="text-xs text-base-content/60 line-clamp-2 mt-1">' + (post.postAbstract || '') + '</p>' +
                        '<div class="flex items-center justify-between mt-3 text-xs text-base-content/50">' +
                            '<div class="flex items-center gap-1">' +
                                '<span class="iconify" data-icon="heroicons:user-circle" data-width="14"></span>' +
                                '<span>' + (post.authorName || '匿名') + '</span>' +
                            '</div>' +
                            '<span>' + timeStr + '</span>' +
                        '</div>' +
                        '<div class="flex items-center gap-3 mt-2 text-xs text-base-content/40">' +
                            '<span class="flex items-center gap-1"><span class="iconify" data-icon="heroicons:heart" data-width="12"></span>' + (post.likeCount || 0) + '</span>' +
                            '<span class="flex items-center gap-1"><span class="iconify" data-icon="heroicons:eye" data-width="12"></span>' + (post.viewCount || 0) + '</span>' +
                            '<span class="flex items-center gap-1"><span class="iconify" data-icon="heroicons:chat-bubble-left" data-width="12"></span>' + (post.commentCount || 0) + '</span>' +
                        '</div>' +
                    '</div>' +
                '</div>';
            }).join('');
        } catch (e) {
            console.error('加载相关帖子失败:', e);
            container.innerHTML = '<div class="col-span-full text-center py-4 text-sm text-base-content/50">加载相关帖子失败</div>';
        }
    }
    ...
    // Render chart based on type - 暴露到全局供其他模块调用
    window.renderChart = function (chartType) {
    ...
    function initializeChart() {
        window.currentChartType = 'nodeTypePie';
        document.getElementById('selectedChartType').textContent = chartTypes[window.currentChartType].name;
        window.renderChart(window.currentChartType);
    }
    ...
    window.updateNodeDetailPanel = function (node) {
        ...
        let description = node.description || fullNodeData?.description;

        // Check if we need to fetch details (if description is undefined)
        const shouldFetch = typeof description === 'undefined';

        // 获取该节点的关联节点
        const relatedNodes = typeof getRelatedNodesFromData === 'function' ? getRelatedNodesFromData(nodeId) : [];
        const connectionCount = typeof getNodeConnectionCount === 'function' ? getNodeConnectionCount(nodeId) : 0;

        // 获取节点颜色
        const nodeColor = typeof getNodeColor === 'function' ? getNodeColor(node.type || 'default') : '#888';
        ...
        container.innerHTML = `
            <div class="space-y-4">
                ...
                <div class="bg-base-100 rounded-lg p-3">
                    ...
                    <div id="node-desc-${nodeId}" class="text-sm text-base-content/80">
                        ${description || (shouldFetch ? '<span class="loading loading-dots loading-xs"></span> 正在加载详情...' : '<span class="text-base-content/40">暂无简介</span>')}
                    </div>
                </div>
                    ...
                    <div class="space-y-1 max-h-40 overflow-y-auto">
                        ${relatedNodes.slice(0, 10).map(rn => `
                            <div class="flex items-center gap-2 p-2 bg-base-100 rounded hover:bg-base-300 cursor-pointer text-sm" onclick="showCenteredGraphView('${rn.id}')">
                                <div class="w-2 h-2 rounded-full" style="background-color: ${typeof getNodeColor === 'function' ? getNodeColor(rn.type || 'default') : '#888'};"></div>
        ...
        if (shouldFetch && currentGraphData.id) {
            fetch(`/api/graph/${currentGraphData.id}/nodes/${nodeId}`)
                .then(res => {
                    if (!res.ok) throw new Error('Failed to load node info');
                    return res.json();
                })
                .then(data => {
                    const descEl = document.getElementById(`node-desc-${nodeId}`);
                    if (descEl) {
                        if (data.description) {
                            descEl.textContent = data.description;
                            // Cache it for future clicks
                            if (fullNodeData) fullNodeData.description = data.description;
                        } else {
                            descEl.innerHTML = '<span class="text-base-content/40">暂无简介</span>';
                            if (fullNodeData) fullNodeData.description = '';
                        }
                    }
                })
                .catch(err => {
                    ...
                    const descEl = document.getElementById(`node-desc-${nodeId}`);
                    if (descEl) descEl.innerHTML = '<span class="text-error text-xs">加载详情失败</span>';
                });
        }
    };
...
(function () {
    ...
    const SidebarManager = {
        init: function () {
            // 绑定点击事件 (移除 HTML 中的 onclick 依赖)
            this.bindClick('leftSidebarToggle', () => this.toggleLeft());
            this.bindClick('rightSidebarToggle', () => this.toggleRight());

            // 初始化拖拽
            this.initResize('leftResizeHandle', 'leftSidebar', 'left');
            this.initResize('rightResizeHandle', 'rightSidebar', 'right');
        ...
        bindClick: function (id, handler) {
            ...
            if (btn) {
                ...
                // Clone to strip other listeners if any (optional, but keep for safety)
                const newBtn = btn.cloneNode(true);
                // After clone, onclick attribute might be copied back if not removed from original before cloning?
                // Actually cloneNode(true) copies attributes.
        ...
        updateIcons: function () {
            ...
            if (leftSidebar && leftIcon) {
                leftIcon.setAttribute('data-icon', leftSidebar.classList.contains('collapsed') ? 'heroicons:chevron-right' : 'heroicons:chevron-left');
            }

            const rightSidebar = document.getElementById('rightSidebar');
            const rightIcon = document.getElementById('rightToggleIcon');
            if (rightSidebar && rightIcon) {
                rightIcon.setAttribute('data-icon', rightSidebar.classList.contains('collapsed') ? 'heroicons:chevron-left' : 'heroicons:chevron-right');
            }
        },
        ...
        initResize: function (handleId, sidebarId, direction) {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                if (direction === 'left') {
                    // 左侧栏：宽度 = 鼠标X - 左边界X
                    // 确保 rect.left 是相对视口的，e.clientX 也是
                    newWidth = e.clientX - rect.left;
                } else {
                    // 右侧栏：宽度 = 右边界X - 鼠标X
                    // rect.right 在拖拽过程中是固定的吗？
                    // 右侧栏通常是靠右对齐的。如果宽度增加，它向左扩展。
                    // 所以 rect.right 应该是固定的（锚点）。
    ...
    // 暴露全局函数以兼容旧的 onclick 调用 (防止报错)
    window.toggleLeftSidebar = () => SidebarManager.toggleLeft();
    window.toggleRightSidebar = () => SidebarManager.toggleRight();
